...
class FSC147Dataset(Dataset):
    def __init__(self, data_dir, data_list, scaling, box_number=3, scale_number=20, min_size=384, max_size=1584,
    ...
    def __getitem__(self, idx):
        ...
        if file_name in self.images:
            ...
            density_path = os.path.join(self.data_dir,
                                        'gt_density_map_adaptive_384_VarV2/' + file_name.replace('jpg', 'npy'))
            ...
            if points.shape[0] > 0:
                ...
                text_prompt = get_image_class(file_name)

                best_example_images, best_example_boxes = SamClipModel(
                    SamModelPath="./checkpoints/bmnet+_pretrained/sam_vit_h_4b8939.pth"
                    , image=img, text_prompt=text_prompt)

                for idx, box in enumerate(best_example_boxes):
                    x1, y1, x2, y2 = box
                    patches.append(self.query_transform(best_example_images[idx]))
                    # calculate scale
